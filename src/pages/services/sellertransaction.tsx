...
import { Check, ChevronsUpDown, Loader2, History, PlusCircle, Search, TrendingUp, Building, TrendingDown, CreditCard, Wallet, MonitorCheck } from "lucide-react";
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [sellersRes, txsRes] = await Promise.all([
                ...
                transactionApi.getAll(entId, undefined)
            ]);

            // Filter transactions for sellers specifically
            const sellerTxs = txsRes.filter((tx: Transaction) => tx.sellerId);
