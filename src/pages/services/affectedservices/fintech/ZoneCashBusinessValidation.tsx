...
export default function ZoneCashBusinessValidation() {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      toast.error('Erreur lors du chargement des comptes en attente');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleValidate = async (
    ...
    try {
      ...
      toast.success(`Compte ${status === 'ACTIVE' ? 'Approuvé' : 'Rejeté'} avec succès`);
      fetchAccounts();
    } catch (err: any) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-black text-slate-800">Validation des Comptes Business</h1>
      </div>

      {accounts.length === 0 ? (
        <Card className="shadow-none border-dashed bg-slate-50">
          <CardContent className="p-12 text-center text-slate-500">
            Aucun compte business en attente de validation.
          </CardContent>
        </Card>
      ) : (
          ...
          {accounts.map(acc => (
            <Card key={acc.id} className="shadow-sm">
              <CardHeader className="pb-3 border-b border-slate-100">
                ...
                <p className="text-sm text-slate-500">Propriétaire: {acc.user?.firstName} {acc.user?.lastName}</p>
                <p className="text-sm text-slate-500">Téléphone: {acc.user?.phoneNumber}</p>
              </CardHeader>
        ...
        >
          ...
          >
            ...
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 bg-white">
              <h3 className="text-lg font-black text-slate-800">Document d'enregistrement</h3>
              <button 
                onClick={() => setSelectedDocUrl(null)}
      ...
      {rejectingAccountId && (
        ...
        >
          <div 
            ...
            onClick={e => e.stopPropagation()}
          >
            <h3 className="text-xl font-black text-slate-800 mb-2">Motif du rejet</h3>
            <p className="text-sm text-slate-500 mb-4">
              Veuillez cocher les éléments incorrects et saisir la raison du rejet.
            </p>
            ...
            <div className="space-y-3 mb-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                />
                <span className="text-sm font-bold text-slate-700">Nom du business incorrect</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm font-bold text-slate-700">Document invalide ou expiré</span>
              </label>
            </div>
              ...
              <Button
                onClick={async () => {
                  if (!rejectNameFlag && !rejectDocumentFlag) {
                    toast.error("Veuillez cocher au moins un élément à corriger.");
                    return;
                  }
